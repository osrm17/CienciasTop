...
public class ProductoRestController {
    ...
    public ResponseEntity<?> show(@PathVariable String codigo) {
        ...
        if(producto == null) {
            response.put("mensaje", "¡El producto con código ".concat(codigo.toString().concat(" no existe en la base de datos!")));
    ...
    public ResponseEntity<?> update(@RequestBody Producto nuevo, @PathVariable String codigo) {
        ...
        //Error con el id ingresado.
        if(actual == null) {
            response.put("mensaje", "Error: no se puede editar el producto con código:".concat(codigo.toString().concat(" porque no existe en la base de datos.")));
            return new ResponseEntity<Map<String, Object>>(response, HttpStatus.NOT_FOUND);
        }
        try {
